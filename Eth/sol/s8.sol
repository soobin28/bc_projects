...
contract BasicString
{
    ...
    function byteArrayToString(byte[] memory src) public pure returns(string memory)
    {
        // byte[] -> bytes -> string
        // byte[] -> bytes
        // 입력한  byte 배열의 크기만틈 bytes의 크기를 잡아준다.
        bytes memory buffer = new bytes(src.length);
        // 인덱스 별로 카피
        for(uint8 i=0;i<src.length;i++){
            buffer[i] = src[i];
        }
        // 문자열변환, 모든 데이터는 bytes에 담겼으므로, string 변환
    ...
    // test
    // string -> bytes -> byte[] -> bytes -> string완성
    function testATBATS(string memory src) public pure returns (string memory)
    {
    ...
    // outpur :string
    // 입력은 문자열을 이어 붙여서 리턴하는 함수
    //문자열을 bytes 변환하여 2개 합한 크기로 버퍼를 만들어서 차례대로 복사후 문자열화 시키면 완성
    // 1. 함수의 외형부터 완성
    function addString (string memory a, string memory b) public pure returns(string memory)
    {
        ...
        // 3. 2개 합한 크기로 버퍼를 만들어 -> 동적생성(dynamic create)
