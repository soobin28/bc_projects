...
router.get('/', function(req, res, next) {
  ...
  // 렌더링의 대상 ejs 파일은 무조건 views 밑에서 찾는다.
  res.render('index', { title: '귀요운 수비니', name:req.session.name });
});

// ~/search
router.get('/search',(req, res, next)=>{
  // 1. 사이트가 요청이 들어오면, noSell테이블에서 이름순으로 젠체 데이터를 가져온다
